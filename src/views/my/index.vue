...
div.pb-20.pt-11.bg-gray-100.h-full 
  ...
  div.px-4
    ...
    div.diamond-box.w-full.h-24.rounded.flex.justify-between
        div.flex.flex-col.text-white.justify-center.pl-4
            div.text-sm 我的球钻
            div.flex.items-end
                span.diamond.block.h-7.w-7
                span.text-3xl.font-medium.mx-1.translate-y 100
                span.text-xs 个
        div.flex.items-center.mr-4
            button.w-16.h-6.bg-red-600.rounded.text-white.text-xs 充值
    ul.flex.bg-white.rounded.my-2.px-4.py-3
        li.flex.flex-col.items-center.mr-4(v-for="item in tabList" :key="item.id" @click="onRoute(item)")
            div.w-7.h-7.mb-2
                van-image(:src="Util.$require(`assets/imgs/my/icon_${item.icon}.png`)")
            div.text-xs.text-red-600 {{item.text}}
...
const cellList = ref([
    ...
    {id: 4, text: '在线客服', icon: 'service', router: '' },
    {id: 5, text: '常见问题', icon: 'ques', router: '/my/faq' }
])
