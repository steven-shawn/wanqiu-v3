...
div.h-11.flex.bg-white.w-screen.justify-between.items-center.px-3.fixed.bottom-0
  div.flex.flex-1.bg-gray-200.h-9.mr-2.rounded.items-center.justify-between
    ...
    p.flex.flex-1.items-center
      ...
      p.text-blue.text-xs(@click="onRoute" v-if="!isLogin") 登陆/注册
      input.block.w-4.h-4.bg-transparent.flex-1.h-9.text-xs.pl-1(placeholder="发弹幕,参与互动" v-model="state.value" :disabled="!isLogin")
      span.block.w-4.h-4.bg-blue-600.ml-2
      span.block.w-6.h-6.mx-2(@click="onSend(state.value)" v-if="isLogin")
        img(src="./send@2x.png")
...
import { useStore } from 'vuex';
  ...
  const isLogin = computed(() => {
    return store.state.user.userInfo.access_token
  })
